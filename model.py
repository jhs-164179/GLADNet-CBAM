...
def FG(input_im):
  with tf.compat.v1.variable_scope('FG'):
    input_rs = tf.compat.v1.image.resize_nearest_neighbor(input_im, (96, 96))

    p_conv1 = tf.compat.v1.layers.conv2d(input_rs, 64, 3, 2, padding='same', activation=tf.compat.v1.nn.relu) # 48
    p_conv2 = tf.compat.v1.layers.conv2d(p_conv1,  64, 3, 2, padding='same', activation=tf.compat.v1.nn.relu) # 24
    p_conv3 = tf.compat.v1.layers.conv2d(p_conv2,  64, 3, 2, padding='same', activation=tf.compat.v1.nn.relu) # 12
    p_conv4 = tf.compat.v1.layers.conv2d(p_conv3,  64, 3, 2, padding='same', activation=tf.compat.v1.nn.relu) # 6
    p_conv5 = tf.compat.v1.layers.conv2d(p_conv4,  64, 3, 2, padding='same', activation=tf.compat.v1.nn.relu) # 3
    p_conv6 = tf.compat.v1.layers.conv2d(p_conv5,  64, 3, 2, padding='same', activation=tf.compat.v1.nn.relu) # 1

    p_deconv1 = tf.compat.v1.image.resize_nearest_neighbor(p_conv6, (3, 3))
    p_deconv1 = tf.compat.v1.layers.conv2d(p_deconv1, 64, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    p_deconv1 = p_deconv1 + p_conv5
    p_deconv2 = tf.compat.v1.image.resize_nearest_neighbor(p_deconv1, (6, 6))
    p_deconv2 = tf.compat.v1.layers.conv2d(p_deconv2, 64, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    p_deconv2 = p_deconv2 + p_conv4
    p_deconv3 = tf.compat.v1.image.resize_nearest_neighbor(p_deconv2, (12, 12))
    p_deconv3 = tf.compat.v1.layers.conv2d(p_deconv3, 64, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    p_deconv3 = p_deconv3 + p_conv3
    p_deconv4 = tf.compat.v1.image.resize_nearest_neighbor(p_deconv3, (24, 24))
    p_deconv4 = tf.compat.v1.layers.conv2d(p_deconv4, 64, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    p_deconv4 = p_deconv4 + p_conv2
    p_deconv5 = tf.compat.v1.image.resize_nearest_neighbor(p_deconv4, (48, 48))
    p_deconv5 = tf.compat.v1.layers.conv2d(p_deconv5, 64, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    p_deconv5 = p_deconv5 + p_conv1
    p_deconv6 = tf.compat.v1.image.resize_nearest_neighbor(p_deconv5, (96, 96))
    p_deconv6 = tf.compat.v1.layers.conv2d(p_deconv6, 64, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)

    p_output = tf.compat.v1.image.resize_nearest_neighbor(p_deconv6, (tf.compat.v1.shape(input_im)[1], tf.compat.v1.shape(input_im)[2]))

    a_input = tf.compat.v1.concat([p_output, input_im], axis=3)
    a_conv1 = tf.compat.v1.layers.conv2d(a_input, 128, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    a_conv2 = tf.compat.v1.layers.conv2d(a_conv1, 128, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    a_conv3 = tf.compat.v1.layers.conv2d(a_conv2, 128, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    a_conv4 = tf.compat.v1.layers.conv2d(a_conv3, 128, 3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    a_conv5 = tf.compat.v1.layers.conv2d(a_conv4, 3,   3, 1, padding='same', activation=tf.compat.v1.nn.relu)
    return a_conv5


def GLADNet_CBAM(input_im):
  with tf.compat.v1.variable_scope('FG'):
    nb_filter = 64
...
class lowlight_enhance(object):
    def __init__(self, sess, model='gladnet'):
        self.sess = sess
        self.base_lr = 0.001

        self.input_low = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, None, None, 3], name='input_low')
        self.input_high = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, None, None, 3], name='input_high')

        if model=='gladnet':
            self.output = FG(self.input_low)
        else:
            # GLADNet + CBAM
            self.output = GLADNet_CBAM(self.input_low)
        self.loss = tf.compat.v1.reduce_mean(tf.compat.v1.abs((self.output - self.input_high) * [[[[0.11448, 0.58661, 0.29891]]]]))
    ...
    def load(self, saver, ckpt_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            full_path = tf.compat.v1.train.latest_checkpoint(ckpt_dir)
            try:
