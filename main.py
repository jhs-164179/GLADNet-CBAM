...
parser.add_argument('--model', dest='model', default='gladnet_cbam', help='model selection for train and test')
...
def lowlight_train(lowlight_enhance):
    ...
    # reducing the dataset size (due to the resource constraint)
    train_low_data_names = train_low_data_names[:200]

    train_high_data_names = glob('./data/train/Normal/*.png')#./data/train/normal/*.png')
    train_high_data_names.sort()

    # reducing the dataset size (due to the resource constraint)
    train_high_data_names = train_high_data_names[:200]
...
def main(_):
    if args.use_gpu:
        ...
        with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
            model = lowlight_enhance(sess, model=args.model)
            if args.phase == 'train':
